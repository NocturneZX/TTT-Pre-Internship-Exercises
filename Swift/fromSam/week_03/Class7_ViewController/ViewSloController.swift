...
class ViewSloController: UIViewController {
    ...
    */
    ...
    @IBOutlet weak var mainPanelTextView: UITextView!
    ...
    override func loadView()
    {
        ...
        var nibs = NSBundle.mainBundle().loadNibNamed("ViewSloController", owner: self, options: nil)
    ...
    @IBAction func toggleColor(sender: AnyObject) {
        var selected = sender.selectedSegmentIndex
        if selected == 0 {
            mainPanelView.backgroundColor = UIColor.cyanColor()
        } else {
            mainPanelView.backgroundColor = UIColor.yellowColor()
        }
    }
    @IBAction func slideTextAlpha(sender: AnyObject) {
//        mainPanelTextView.alpha = 50
    }
    
    @IBAction func switchTextVisibility(sender: AnyObject) {
        if mySwitch.on {
            mainPanelTextView.hidden = false
        } else {
            mainPanelTextView.hidden = true
        }
    }
    
    //  slider: makeapppie.com/tag/make-color-sliders-in-swift
    
// *** The nib and the xib are the same thing.. referencing based on swift/Obj C.  The View Controller .swft needs to have the same name as the as the View .xib
